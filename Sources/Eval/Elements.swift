...
/// `MatchElement`s are used by `Matcher` instances to be able to recognise patterns. Currently, the two main kind of `MatchElement` classes are `Keyword`s and `Variable`s
...
public class Keyword : MatchElement {
    ...
    /// The `Keyword` always results in String values, after the matching
...
/// A special subclass of the `Keyword` class, which initialises a `Keyword` with an opening type. Usually used for opening parentheses: OpenKeyword("[")
...
/// A special subclass of the `Keyword` class, which initialises a `Keyword` with an closing type. Usually used for closing parentheses: CloseKeyword("]")
...
/// Generic superclass of `Variable`s which are aware of their `Interpreter` classes, as they use it when mapping their values
public class GenericVariable<T, E: Interpreter> : VariableProtocol, MatchElement {
    ...
    /// `shortest` provides information whether the match should be exhaustive or just use the shortest possible matching string (even zero characters in some edge cases). This depends on the surrounding `Keyword` instances in the containing collection.
    /// If `interpreted` is false, the value of the recognised placeholder will not be processed. In case of true, it will be evaluated, using the `interpreterForEvaluatingVariables` property of the interpreter isntance
    ...
    /// The final `map` block is optional. If provided, then the result of the evaluated variable will be ran through this map function, transforming its value. By default the map tries to convert the matched value to the expected type, using the `as?` operator.
