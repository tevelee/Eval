...
public class Pattern<T, I: Interpreter> {
    ...
    public init(_ elements: [PatternElement],
        ...
        self.elements = Pattern.elementsByReplacingTheLastVariableNotToBeShortestMatch(in: elements, isBackward: isBackward)
    }
    ...
    static func elementsByReplacingTheLastVariableNotToBeShortestMatch(in elements: [PatternElement], isBackward: Bool) -> [PatternElement] {
        var elements = elements
        let index = isBackward ? elements.startIndex : elements.index(before: elements.endIndex)
        
        if let last = elements[index] as? GenericVariable<I.EvaluatedType, I> {
            ...
            let longest = GenericVariable<Any, I>(last.name, shortest: false, interpreted: last.interpreted, acceptsNilValue: last.acceptsNilValue, map: last.performMap)
            elements.remove(at: index)
            elements.insert(longest, at: index)
        }
        return elements
    ...
    func matches(string: String, from start: Int = 0, interpreter: I, context: Context) -> MatchResult<T> {
        ...
        let exec = Matcher(elements: elements, processor: processor, isBackward: isBackward)
